...
class DATA_LOADER(object):
    ...
    def read_matdataset(self, opt):
        ...
        # test_original_loc = torch.arange(len(label_number_list)-100, len(label_number_list))
        # test_new_loc = torch.arange(len(label_number_list))

        # original_loc = [index for index, value in enumerate(label_number_list) if value in [0,1,2]]
        # trainval_loc = original_loc[:-100]
        # test_original_loc = original_loc[-100:]
        # test_new_loc = [index for index, value in enumerate(label_number_list) if value in [3,4]]

        # self.train_feature = torch.from_numpy(feature[trainval_loc]).float()
        # self.train_label = torch.tensor([label_number_list[idx] for idx in trainval_loc]).long()
        # self.test_new_feature = torch.from_numpy(feature[test_new_loc]).float()
        # self.test_new_label = torch.tensor([label_number_list[idx] for idx in test_new_loc]).long()
        # self.test_original_feature = torch.from_numpy(feature[test_original_loc]).float()
        # self.test_original_label = torch.tensor([label_number_list[idx] for idx in test_original_loc]).long()

        test_new_loc = torch.arange(len(label))
        self.test_new_feature = torch.from_numpy(feature[test_new_loc]).float()
        self.test_new_label = torch.tensor([label[idx] for idx in test_new_loc]).long()
        ...
        test_original_loc = torch.arange(len(label)-50, len(label))
        # torch.Size([752, 2048])
        self.train_feature = torch.from_numpy(feature[trainval_loc]).float()
        self.train_label = torch.tensor([label[idx] for idx in trainval_loc]).long()
        self.test_original_feature = torch.from_numpy(feature[test_original_loc]).float()
        self.test_original_label = torch.tensor([label[idx] for idx in test_original_loc]).long()
        ...
        self.originalclasses = torch.from_numpy(np.unique(self.train_label.numpy()))
        self.newclasses = torch.from_numpy(np.unique(self.test_new_label.numpy()))

        if opt.zst:
            self.newclasses = self.newclasses + len(self.originalclasses)
            self.originalclasses = torch.arange(len(self.originalclasses))

        print(self.newclasses)
        print(self.originalclasses)

        self.nclass = len(self.originalclasses) + len(self.newclasses)
        self.ntrain = self.train_feature.size()[0]
        self.train_mapped_label = map_label(self.train_label, self.originalclasses)
