...
for _lr, _beta1, _nepoch, _batch_size, _embed_dim, _num_layers, _new_rate, _train_sample_missing_rate in params:
    ...
    for seed in seedlist:
        ...
        if os.path.isfile(model_path) and not opt.overwrite_base_classifier_cache:
            ...
            base_model = BASECLASSIFIER(data.train_feature, data_preparation.map_label(data.train_label, data.originalclasses), data, data.nclass, opt.cuda, seedinfo=seed, _lr=_lr, _beta1=_beta1, _nepoch=opt.classifier_nepoch, _batch_size=_batch_size, _embed_dim=_embed_dim, _num_layers=_num_layers, opt=opt).fit()
        ...
        if opt.source_only_benchmark:
            Sonly = sonly.SourceOnly(_train_X=data.train_feature, _train_Y=data_preparation.map_label(data.train_label, data.originalclasses), data_loader=data,  _nclass=data.nclass, _cuda=opt.cuda, seedinfo=seed, _lr=_lr, _beta1=_beta1, _nepoch=_nepoch, _batch_size=_batch_size, _embed_dim=_embed_dim, _num_layers=_num_layers, opt=opt)
            ...
            acc_gzsl, acc_original, acc_new, H, acc_new_only = Sonly.acc_gzsl, Sonly.acc_original, Sonly.acc_new, Sonly.H, Sonly.acc_new_zsl
        # baseline ConSE
        elif opt.conse_benchmark:
            bs = opt.batch_size[0]
            ConSE = conse.ConSE(_train_X=data.train_feature, _train_Y=data_preparation.map_label(data.train_label, data.originalclasses), data_loader=data, _nclass=data.nclass, _cuda=opt.cuda, seedinfo=seed,
                                _lr=_lr, _beta1=_beta1, _nepoch=_nepoch, _batch_size=bs, _embed_dim=_embed_dim, _num_layers=_num_layers, opt=opt)
            method_name = "conse"
            acc_gzsl, acc_original, acc_new, H, acc_new_only = ConSE.acc_gzsl, ConSE.acc_original, ConSE.acc_new, ConSE.H, ConSE.acc_new_zsl
        # baseline COSTA
        elif opt.costa_benchmark:
            COSTA = costa.COSTA(_train_X=data.train_feature, _train_Y=data_preparation.map_label(data.train_label, data.originalclasses), data_loader=data, _nclass=data.nclass, _cuda=opt.cuda, seedinfo=seed,
                                _lr=_lr, _beta1=_beta1, _nepoch=_nepoch, _batch_size=_batch_size, _embed_dim=_embed_dim, _num_layers=_num_layers, opt=opt)
            method_name = "costa"
            acc_gzsl, acc_original, acc_new, H, acc_new_only = COSTA.acc_gzsl, COSTA.acc_original, COSTA.acc_new, COSTA.H, COSTA.acc_new_zsl
        # baseline VGSE
        elif opt.vgse_baseline:
            VGSE = vgse.VGSE_CRM(_train_X=data.train_feature, _train_Y=data_preparation.map_label(data.train_label, data.originalclasses), data_loader=data, _nclass=data.nclass, _cuda=opt.cuda, seedinfo=seed,
                                _lr=_lr, _beta1=_beta1, _nepoch=_nepoch, _batch_size=_batch_size, _embed_dim=_embed_dim, _num_layers=_num_layers, opt=opt)
            method_name = "vgse"
            acc_gzsl, acc_original, acc_new, H, acc_new_only = VGSE.acc_gzsl, VGSE.acc_original, VGSE.acc_new, VGSE.H, VGSE.acc_new_zsl
        # baseline DANN
        elif opt.dann_benchmark:
            DANN = dann.DANN(_train_X=data.train_feature, _train_Y=data_preparation.map_label(data.train_label, data.originalclasses), data_loader=data, _nclass=data.nclass, _cuda=opt.cuda, seedinfo=seed,
                                _lr=_lr, _beta1=_beta1, _nepoch=_nepoch, _batch_size=_batch_size, _embed_dim=_embed_dim, _num_layers=_num_layers, opt=opt)
            ...
            method_name = "dann"
            acc_gzsl, acc_original, acc_new, H, acc_new_only = DANN.acc_gzsl, DANN.acc_original, DANN.acc_new, DANN.H, DANN.acc_new_zsl
        # baseline ADDA
        elif opt.adda_benchmark:
            ADDA = adda.ADDA(_train_X=data.train_feature, _train_Y=data_preparation.map_label(data.train_label, data.originalclasses), data_loader=data, _nclass=data.nclass, _cuda=opt.cuda, seedinfo=seed,
                                _lr=_lr, _beta1=_beta1, _nepoch=_nepoch, _batch_size=_batch_size, _embed_dim=_embed_dim, _num_layers=_num_layers, opt=opt)
            method_name = "adda"
            acc_gzsl, acc_original, acc_new, H, acc_new_only = ADDA.acc_gzsl, ADDA.acc_original, ADDA.acc_new, ADDA.H, ADDA.acc_new_zsl
        # baseline TPDS
        elif opt.tpds_benchmark:
            TPDS = tpds.TPDS(_train_X=data.train_feature, _train_Y=data_preparation.map_label(data.train_label, data.originalclasses), data_loader=data, _nclass=data.nclass, _cuda=opt.cuda, seedinfo=seed,
                                _lr=_lr, _beta1=_beta1, _nepoch=_nepoch, _batch_size=_batch_size, _embed_dim=_embed_dim, _num_layers=_num_layers, opt=opt)
            ...
            method_name = "tpds"
            acc_gzsl, acc_original, acc_new, H, acc_new_only = TPDS.acc_gzsl, TPDS.acc_original, TPDS.acc_new, TPDS.H, TPDS.acc_new_zsl
        # baseline G2KD
        elif opt.g2kd_benchmark:
            G2KD = g2kd.G2KD(_train_X=data.train_feature, _train_Y=data_preparation.map_label(data.train_label, data.originalclasses), data_loader=data, _nclass=data.nclass, _cuda=opt.cuda, seedinfo=seed,
                                _lr=_lr, _beta1=_beta1, _nepoch=_nepoch, _batch_size=_batch_size, _embed_dim=_embed_dim, _num_layers=_num_layers, opt=opt)
            method_name = "g2kd"
            acc_gzsl, acc_original, acc_new, H, acc_new_only = G2KD.acc_gzsl, G2KD.acc_original, G2KD.acc_new, G2KD.H, G2KD.acc_new_zsl
        else:
            MODEL = dual_align.Joint(data.train_feature, data_preparation.map_label(data.train_label, data.originalclasses), data, data.nclass, opt.cuda, seedinfo=seed,
                                    _lr=_lr, _beta1=_beta1, _nepoch=_nepoch, _batch_size=_batch_size, _embed_dim=_embed_dim, _num_layers=_num_layers, _new_rate=_new_rate, opt=opt)
            MODEL.fit()
            ...
            method_name = "ours"
            if opt.zst:
                acc_new_only, acc_new = MODEL.acc_target, MODEL.acc_zst_new
            else:
                acc_gzsl, acc_original, acc_new, H, acc_new_only = MODEL.acc_gzsl, MODEL.acc_original, MODEL.acc_new, MODEL.H, MODEL.acc_new_zsl    
        ...
        if opt.zst:
            print(f"I-ZSL accuracy from {opt.zstfrom} transfer: {acc_new_only*100:.2f}%.")
            print(f"new accuracy (not H) I-GZSL from {opt.zstfrom} transfer: {acc_new*100:.2f}%.")
            if opt.accuracy_only_csv:
                res = {"method": method_name, "source_dataset": opt.zstfrom, "target_dataset": opt.dataset, "dataset": opt.dataset, "zstfrom": opt.zstfrom, "seed": seed, "train_sample_missing_rate": _train_sample_missing_rate, "view_percent": opt.view_percent, "view_num": opt.view_num, "selected_view_indices": getattr(opt, "selected_view_indices", ""), "view_error_percent": opt.view_error_percent, "error_view_indices": getattr(opt, "error_view_indices", ""), "zsl_acc": f"{acc_new_only*100:.1f}"}
            else:
                res = {"method": method_name, "source_dataset": opt.zstfrom, "target_dataset": opt.dataset, "dataset": opt.dataset, "zstfrom": opt.zstfrom, "seed": seed, "image_emb":opt.image_embedding, "factual_emb": opt.factual_branch, "intervention_emb": opt.intervention_branch, "llm": opt.llm, "emb_model": opt.class_embedding, "embed_dim": _embed_dim, "class_num": opt.class_reduction_ablation, "train_sample_missing_rate": _train_sample_missing_rate, "view_percent": opt.view_percent, "view_num": opt.view_num, "selected_view_indices": getattr(opt, "selected_view_indices", ""), "view_error_percent": opt.view_error_percent, "error_view_indices": getattr(opt, "error_view_indices", ""), "zsl_acc": f"{acc_new_only*100:.1f}", "new": f"{acc_new*100:.1f}"}
        else:
            print(f"I-ZSL (new only) Acc={acc_new_only*100:.1f}%")
            print(f"I-GZSL (original & new): new={acc_new*100:.1f}%, original={acc_original*100:.1f}%, H={H*100:.1f}")
            if opt.accuracy_only_csv:
                res = {"method": method_name, "dataset": opt.dataset, "seed": seed, "train_sample_missing_rate": _train_sample_missing_rate, "view_percent": opt.view_percent, "view_num": opt.view_num, "selected_view_indices": getattr(opt, "selected_view_indices", ""), "view_error_percent": opt.view_error_percent, "error_view_indices": getattr(opt, "error_view_indices", ""), "zsl_acc": f"{acc_new_only*100:.1f}"}
            else:
                res = {"method": method_name, "dataset": opt.dataset, "seed": seed, "image_emb":opt.image_embedding, "factual_emb": opt.factual_branch, "intervention_emb": opt.intervention_branch, "llm": opt.llm, "emb_model": opt.class_embedding, "embed_dim": _embed_dim, "class_num": opt.class_reduction_ablation, "train_sample_missing_rate": _train_sample_missing_rate, "view_percent": opt.view_percent, "view_num": opt.view_num, "selected_view_indices": getattr(opt, "selected_view_indices", ""), "view_error_percent": opt.view_error_percent, "error_view_indices": getattr(opt, "error_view_indices", ""), "zsl_acc": f"{acc_new_only*100:.1f}", "u": f"{acc_new*100:.1f}", "s": f"{acc_original*100:.1f}", "H": f"{H*100:.1f}", "new_rate": _new_rate}
        ...
        new_zsl_seeds_avg.append(acc_new_only)
        acc_new_seeds_avg.append(acc_new)
        if not opt.zst:
            acc_gzsl_seeds_avg.append(acc_gzsl)
            acc_original_seeds_avg.append(acc_original)
            H_seeds_avg.append(H)

    accs_new_only.append(torch.std_mean(torch.stack(new_zsl_seeds_avg), dim=0, unbiased=False)[1])
    accs_new_only_std.append(torch.std_mean(torch.stack(new_zsl_seeds_avg), dim=0, unbiased=False)[0])
    accs_new.append(torch.std_mean(torch.stack(acc_new_seeds_avg), dim=0, unbiased=False)[1])
    accs_new_std.append(torch.std_mean(torch.stack(acc_new_seeds_avg), dim=0, unbiased=False)[0])
    ...
    if not opt.zst:
        ...
        accs_original.append(torch.std_mean(torch.stack(acc_original_seeds_avg), dim=0, unbiased=False)[1])
        accs_original_std.append(torch.std_mean(torch.stack(acc_original_seeds_avg), dim=0, unbiased=False)[0])
    ...
    avg_res = {
        ...
        "zsl_acc": f"{accs_new_only[-1]*100:.1f}±{accs_new_only_std[-1]*100:.1f}",
        "new": f"{accs_new[-1]*100:.1f}±{accs_new_std[-1]*100:.1f}",
    }
    if not opt.zst:
        avg_res.update({
            "s": f"{accs_original[-1]*100:.1f}±{accs_original_std[-1]*100:.1f}",
...
accs_new_only = torch.stack(accs_new_only)
accs_new_only_std = torch.stack(accs_new_only_std)
accs_new = torch.stack(accs_new)
accs_new_std = torch.stack(accs_new_std)
idx_best_new = torch.argmax(accs_new)

if not opt.zst:
    accs_gzsl = torch.stack(accs_gzsl)
    accs_gzsl_std = torch.stack(accs_gzsl_std)
    accs_original = torch.stack(accs_original)
    accs_original_std = torch.stack(accs_original_std)
    hs = torch.stack(hs)
...
if opt.numSeeds > 1:
    ...
    print(
        f"I-ZSL (new only) Acc={accs_new_only[idx_best_H]*100:.1f}±{accs_new_only_std[idx_best_H]*100:.1f}%")
    print(
        f"I-GZSL (original and new) u={accs_new[idx_best_H]*100:.1f}±{accs_new_std[idx_best_H]*100:.1f}%, s={accs_original[idx_best_H]*100:.1f}±{accs_original_std[idx_best_H]*100:.1f}%, H={hs[idx_best_H]*100:.1f}±{hs_std[idx_best_H]*100:.1f}%")
    print("All experiments over the list of seeds completed.")
    res_avg = {"dataset": opt.dataset, "seed": opt.manualSeed, "att_dim": opt.att_dim, "factual_emb": opt.factual_branch, "intervention_emb": opt.intervention_branch, "llm": opt.llm, "emb_model": opt.class_embedding, "batch_size": opt.batch_size, "embed_dim": opt.embed_dim, "class_num": opt.class_reduction_ablation, "view_num": opt.view_num, "zsl_acc": f"{accs_new_only[idx_best_H]*100:.1f}±{accs_new_only_std[idx_best_H]*100:.1f}%", "u": f"{accs_new[idx_best_H]*100:.1f}±{accs_new_std[idx_best_H]*100:.1f}%", "s": f"{accs_original[idx_best_H]*100:.1f}±{accs_original_std[idx_best_H]*100:.1f}%", "H": f"{hs[idx_best_H]*100:.1f}±{hs_std[idx_best_H]*100:.1f}%"}
    if os.path.exists(opt.avg_save_path):
