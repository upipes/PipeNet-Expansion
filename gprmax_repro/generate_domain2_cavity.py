"""Generate Domain 2 cavity gprMax input files for manual inspection."""
...
def header(fractal_seed: int) -> list[str]:
    return [
        "#title: Domain 2 - Cavity in Wet Silty Clay with Peplinski Modeling",
...
def footer(sample_id: int) -> list[str]:
    geometry_name = f"geometry_views/domain2_cavity_{sample_id:03d}_geometry"
    return [
        "",
...
def generate(out_dir: Path, samples: int, seed: int) -> None:
    ...
    for sample_id in range(samples):
        # Use a different default random stream from Domain 1 cavity while
        # keeping per-sample generation reproducible.
        rng = random.Random(seed + 10000 + sample_id)
        shape_mode, content = make_input(sample_id, rng)
        fractal_seed = FRACTAL_SEED_START + sample_id
        path = out_dir / f"domain2_cavity_{sample_id:03d}.in"
        path.write_text(content, encoding="utf-8")
        manifest.append(
            {
                "sample_id": sample_id,
                "fractal_seed": fractal_seed,
                "random_seed": seed + 10000 + sample_id,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate Domain 2 cavity input files.")
    parser.add_argument("--out-dir", type=Path, default=Path("gprMax-master/domain2_cavity_inputs"))
...
def main() -> None:
    ...
    print(f"Generated {args.samples} Domain 2 cavity input files under {args.out_dir}")
