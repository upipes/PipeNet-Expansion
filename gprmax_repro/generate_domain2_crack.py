"""Generate Domain 2 crack gprMax input files for manual inspection."""
...
def header(fractal_seed: int) -> list[str]:
    return [
        "#title: Domain 2 - Crack in Wet Silty Clay with Peplinski Modeling",
...
def footer(sample_id: int) -> list[str]:
    geometry_name = f"geometry_views/domain2_crack_{sample_id:03d}_geometry"
    return [
        "",
...
def bounded_width(width: float) -> float:
    return clamp(width, CRACK_WIDTH_MIN, CRACK_WIDTH_MAX)
...
def gently_curved_crack(rng: random.Random, direction: int) -> list[str]:
    ...
    for idx in range(segments):
        ...
        lines.append(box_line(x1, y, segment_length * rng.uniform(0.92, 1.08), bounded_width(width * rng.uniform(0.9, 1.1))))
    return lines
...
def segmented_near_horizontal(rng: random.Random) -> list[str]:
    ...
    for idx in range(segments):
        ...
        lines.append(box_line(x1, y_center, segment_length * rng.uniform(0.80, 1.05), bounded_width(width * rng.uniform(0.9, 1.1))))
    return lines
...
def generate(out_dir: Path, samples: int, seed: int) -> None:
    ...
    for sample_id in range(samples):
        rng = random.Random(seed + 20000 + sample_id)
        shape_mode, content = make_input(sample_id, rng)
        fractal_seed = FRACTAL_SEED_START + sample_id
        path = out_dir / f"domain2_crack_{sample_id:03d}.in"
        path.write_text(content, encoding="utf-8")
        manifest.append(
            {
                "sample_id": sample_id,
                "fractal_seed": fractal_seed,
                "random_seed": seed + 20000 + sample_id,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate Domain 2 crack input files.")
    parser.add_argument("--out-dir", type=Path, default=Path("gprMax-master/domain2_crack_inputs"))
...
def main() -> None:
    ...
    print(f"Generated {args.samples} Domain 2 crack input files under {args.out_dir}")
