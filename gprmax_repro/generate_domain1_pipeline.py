"""Generate Domain 1 pipeline gprMax input files for manual inspection."""
...
def header(fractal_seed: int) -> list[str]:
    return [
        "#title: Domain 1 - Pipeline in Ideal Sandy Loam with Peplinski Modeling",
...
def footer(sample_id: int) -> list[str]:
    geometry_name = f"geometry_views/domain1_pipeline_{sample_id:03d}_geometry"
    return [
        "",
...
def generate(out_dir: Path, samples: int, seed: int) -> None:
    ...
    for sample_id in range(samples):
        ...
        fractal_seed = FRACTAL_SEED_START + sample_id
        path = out_dir / f"domain1_pipeline_{sample_id:03d}.in"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate Domain 1 pipeline input files.")
    parser.add_argument("--out-dir", type=Path, default=Path("gprMax-master/domain1_pipeline_inputs"))
...
def main() -> None:
    ...
    print(f"Generated {args.samples} Domain 1 pipeline input files under {args.out_dir}")
