...
class Joint(REGRESSOR):
    def __init__(self, _train_X, _train_Y, data_loader, _nclass, _cuda, seedinfo, train_base=False, _lr=0.001, _beta1=0.5, _nepoch=20, _batch_size=100, _embed_dim=1000, _num_layers=3, _new_rate=0, opt=None):
        ...
        if self.opt.class_reduction_ablation:
            perm = torch.randperm(len(self.originalclasses))
            assert self.opt.class_reduction_ablation in range(1, len(self.originalclasses)+1)
            perm = perm[:self.opt.class_reduction_ablation]
            if self.opt.concatenation and self.opt.conclude_inv:
                training_attributes_f = self.attribute_f[self.originalclasses][perm]
                training_attributes_inv = self.attribute_inv[self.originalclasses][perm]
                ...
                training_weights_inv = placeholder_weights_inv[self.originalclasses][perm]
            else:
                training_attributes = self.attribute_new[self.originalclasses][perm]
                training_weights = self.target_weights[perm]

            temp = self.newclasses
            if self.new_rate:
                perm2 = torch.randperm(len(temp))
                num = len(temp) * self.new_rate // 100
                self.perm2 = perm2[:num]
        
        # ablation
        placeholder_weights_f = torch.zeros(len(self.newclasses), self.target_weights.size(1))
        ...
        if opt.single_autoencoder_baseline:
            if opt.class_reduction_ablation:
                ...
                if self.opt.concatenation and self.opt.conclude_inv:
                    att2weight_dataset = data_preparation.GenericDatasetINV(opt, _input=self.attribute_f[self.originalclasses], _target=self.target_weights, _input_inv=self.attribute_inv[self.originalclasses], _target_inv=placeholder_weights_inv[self.originalclasses], cuda=self.cuda)
                else:
                    att2weight_dataset = data_preparation.GenericDataset(opt, _input=self.attribute_new[self.originalclasses], _target=self.target_weights, cuda=self.cuda)
                self.loader = DataLoader(att2weight_dataset, batch_size=self.batch_size, shuffle=True)
        else:
            if opt.class_reduction_ablation:
                if self.opt.concatenation and self.opt.conclude_inv:
                ...
                if self.opt.concatenation and self.opt.conclude_inv:
                    combined_original_dataset = data_preparation.GenericDatasetINV(opt, _input=self.attribute_f[self.originalclasses], _target=self.target_weights, _input_inv=self.attribute_inv[self.originalclasses], _target_inv=placeholder_weights_inv[self.originalclasses], cuda=self.cuda)
                else:
                    combined_original_dataset = data_preparation.GenericDataset(opt, _input=self.attribute_new[self.originalclasses], _target=self.target_weights, cuda=self.cuda)
            if self.cuda:
                placeholder_weights_f = placeholder_weights_f.cuda()
                placeholder_weights_inv = placeholder_weights_inv.cuda()
            if opt.class_reduction_ablation:
                if self.opt.concatenation and self.opt.conclude_inv:
                    combined_full_dataset = data_preparation.GenericDatasetINV(opt, _input=torch.cat((training_attributes_f, self.attribute_f[self.newclasses])), 
                                                                            _target=torch.cat((training_weights_f, placeholder_weights_f), dim=0),
                                                                            _input_inv=torch.cat((training_attributes_inv, self.attribute_inv[self.newclasses])), 
                                                                            _target_inv=torch.cat((training_weights_inv, placeholder_weights_inv[self.newclasses]), dim=0), cuda=self.cuda)
                else:
                    combined_full_dataset = data_preparation.GenericDataset(opt, _input=torch.cat((training_attributes, self.attribute_new[self.newclasses])), 
                                                                            _target=torch.cat((training_weights, placeholder_weights_f), dim=0), cuda=self.cuda)
            else:
                if self.opt.concatenation and self.opt.conclude_inv:
                    combined_full_dataset = data_preparation.GenericDatasetINV(opt, _input=torch.cat((self.attribute_f[self.originalclasses], self.attribute_f[self.newclasses])), 
                                                                            _target=torch.cat((self.target_weights, placeholder_weights_f), dim=0),
                                                                            _input_inv=torch.cat((self.attribute_inv[self.originalclasses], self.attribute_inv[self.newclasses])), 
                                                                            _target_inv=torch.cat((placeholder_weights_inv[self.originalclasses], placeholder_weights_inv[self.newclasses]), dim=0), cuda=self.cuda)
                else:
                    combined_full_dataset = data_preparation.GenericDataset(opt, _input=torch.cat((self.attribute_new[self.originalclasses], self.attribute_new[self.newclasses])), 
                                                                            _target=torch.cat((self.target_weights, placeholder_weights_f), dim=0), cuda=self.cuda)
            if self.opt.include_new:
                self.loader = DataLoader(combined_full_dataset, batch_size=self.batch_size, shuffle=True)
            else:
                self.loader = DataLoader(combined_original_dataset, batch_size=self.batch_size, shuffle=True)
        ...
        if opt.daegnn:
            ...
            if self.opt.single_autoencoder_baseline:
                self.dae_loader = DataLoader(att2weight_dataset, batch_size=len(self.originalclasses), shuffle=False)
            else:
                self.dae_loader = DataLoader(combined_original_dataset, batch_size=len(self.originalclasses), shuffle=False)
            self.dae = WeightsDAE({
                'gaussian_noise': 0.08,
        ...
        if self.new_rate:
            self.newclasses = self.newclasses[self.perm2]

        self.new_model = model.LINEAR(self.test_original_feature.size(1), len(self.newclasses))
        self.ext_model = model.LINEAR(self.test_original_feature.size(1), len(self.originalclasses) + len(self.newclasses))
        self.ext_model.fc.weight.data[:len(self.originalclasses), :] = self.target_weights[:, :-1]
        self.ext_model.fc.bias.data[:len(self.originalclasses)] = self.target_weights[:, -1]
    ...
    def fit(self):
        run_best_acc_gzsl, run_best_acc_original, run_best_acc_new, run_best_H, run_best_new_zsl = 0, 0, 0, 0, 0
        ...
        for epoch in trange(self.nepoch):
            ...
            if ((not self.opt.strict_eval) or (epoch + 1 == self.nepoch) or breaking) and not self.opt.daegnn:
                ...
                if self.opt.zst:
                    acc_target, acc_zst_new = val_out
                else:
                    acc_gzsl, acc_original, acc_new, H, acc_new_zsl = val_out

                    epoch_info["acc_new_zsl"] = acc_new_zsl
                    epoch_info["H"] = H
                    epoch_info["acc_new_gzsl"] = acc_new
                    epoch_info["acc_original_gzsl"] = acc_original

                    if H >= run_best_H:
                        run_best_acc_gzsl, run_best_acc_original, run_best_acc_new, run_best_H, run_best_new_zsl = acc_gzsl, acc_original, acc_new, H, acc_new_zsl
        ...
        if self.opt.zst:
            self.acc_target, self.acc_zst_new = acc_target, acc_zst_new
        else:
            self.acc_gzsl, self.acc_original, self.acc_new, self.H, self.acc_new_zsl = run_best_acc_gzsl, run_best_acc_original, run_best_acc_new, run_best_H, run_best_new_zsl
